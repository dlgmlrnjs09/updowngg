  ...
  <div class="team">
              ...
              <div class="grid-augments" v-if="isArena">
                <div class="augments-grid">
                  <div class="augment-icon" v-if="player.playerAugment1">
                    <img :src="player.playerAugmentEntity1.iconSmallUrl" alt="Augment 1">
                  </div>
                  <div class="augment-icon" v-if="player.playerAugment2">
                    <img :src="player.playerAugmentEntity2.iconSmallUrl" alt="Augment 2">
                  </div>
                  <div class="augment-icon" v-if="player.playerAugment3">
                    <img :src="player.playerAugmentEntity3.iconSmallUrl" alt="Augment 3">
                  </div>
                  <div class="augment-icon" v-if="player.playerAugment4">
                    <img :src="player.playerAugmentEntity4.iconSmallUrl" alt="Augment 4">
                  </div>
                </div>
              </div>
    ...
    <div class="md:hidden" v-else>
      <div v-for="(team, index) in arenaTeams" :key="index" class="mobile-arena-team">
        <div class="mobile-team-title arena" :class="team[0]?.isWin ? 'win' : 'lose'">
          팀 {{ index + 1 }}
          <span class="mobile-result-text">{{ team[0]?.isWin ? '승리' : '패배' }}</span>
          <span class="mobile-placement-text">{{ getPlacementText(team[0]?.subteamPlacement) }}</span>
        </div>
        <div class="mobile-arena-grid">
          <div v-for="player in team"
               :key="player.puuid"
               class="mobile-arena-grid-item"
               :class="[
                 auth.user?.puuid != profileData.riotAccountInfoEntity.puuid
                 ? 'mobile-someone-else'
                 : {
          ...
          >
            <!-- 메인 정보 영역 -->
            <div class="mobile-arena-main-info">
              <!-- 챔피언 -->
              <div class="mobile-champion-wrapper">
                <div class="mobile-champion-icon">
                  <img :src="player.champProfileIconUrl" :alt="player.champName">
              ...
              <!-- 증강 그리드 영역 - 별도로 분리 -->
              <div class="mobile-grid-augments" v-if="isArena">
              ...
              <!-- 플레이어 정보 -->
              <div class="mobile-arena-player-info">
                <div class="mobile-name-tag">
                  <span class="mobile-name" @click.stop="goSelectedSummonerProfile(player.riotIdGameName, player.riotIdTagline)">
                    {{ player.riotIdGameName }}
                  </span>
              ...
              <!-- 리뷰 정보 -->
              <div class="mobile-arena-review" v-if="player.reviewStatsDto">
                <div class="mobile-review-up">
                  <ThumbsUp class="mobile-thumb-icon up" />
                  <span class="mobile-count">{{ player.reviewStatsDto.upCount }}</span>
                </div>
            ...
            <!-- 데미지 정보 영역 -->
            <div class="mobile-arena-damage">
              <div class="mobile-arena-damage-row">
                <span class="mobile-damage-label">딜량</span>
                <damage-bar
                    :value="player.totalDamageToChampion"
                    :max-value="maxDamage"
                    type="damage"
                    label="가한 피해량"
                    class="mobile-arena-bar"
                />
              </div>
              <div class="mobile-arena-damage-row">
                <span class="mobile-damage-label taken">탱량</span>
                <damage-bar
                    :value="player.totalDamageTaken"
                    :max-value="maxDamageTaken"
                    type="taken"
...
.mobile-arena-main-info {
  grid-area: main-info;
  display: grid;
  grid-template-columns: auto auto 1fr auto;
  grid-template-areas: "champion augments player-info review";
  gap: 6px;
  align-items: center;
}

/* 모바일 증강 그리드 영역 추가 */
.mobile-grid-augments {
  grid-area: augments;
  display: flex;
  align-items: center;
  margin: 0 2px;
...
:deep(.mobile-arena-bar .progress-bg) {
  height: 4px !important;
}

:deep(.mobile-arena-bar .damage-text) {
